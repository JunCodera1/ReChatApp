    ...
    public class MediaBar extends HBox { // MediaBar extends Horizontal Box
        ...
        protected void updatesValues() {
            Platform.runLater(new Runnable() {
                public void run() {
                    // Update the slider's value based on the current time and total duration of the video
                    double currentTime = player.getCurrentTime().toMillis();
                    double totalDuration = player.getTotalDuration().toMillis();
                    if (totalDuration > 0) {
                        time.setValue((currentTime / totalDuration) * 100);
                    }
                }
            });
        }
